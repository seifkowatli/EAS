...
namespace EAS_Servies.Data_Mining_Algorithm
{

  public  class SemesterAnalysis
    {
       public double age;
       public double Mark;
       public int Number;
        ...
        public List<List<double>> ClusterList;

        public SemesterAnalysis()
        {
            ClusterList=new List<List<double>>();
        }

    }
    public class SemesterKMeans
    {
        ...
        static public List<SemesterAnalysis> ShowClustered(double[][] data, int[] clustering, int numClusters, int decimals)
        {
            List<SemesterAnalysis> Result = new List<SemesterAnalysis>();
            List<double> temp1 = new List<double>();


            for (int k = 0; k < numClusters; ++k)
            {
                SemesterAnalysis Temp2 = new SemesterAnalysis();
                Temp2.Number = k + 1;
                for (int i = 0; i < data.Length; ++i)
                {
                ...
                foreach (var item in Temp2.ClusterList)
                {
                    age = age + item[0];
                    mark = mark + item[1];
                   
                }
                Temp2.age = Math.Round(age / Temp2.ClusterList.Count, 2);
                Temp2.Mark = Math.Round(mark / Temp2.ClusterList.Count, 2);
               
                Temp2.Count = Temp2.ClusterList.Count;
                Result.Add(Temp2);
